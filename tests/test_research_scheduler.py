...
@pytest.mark.asyncio
async def test_research_scheduler_run_once_marks_market_data_freshness_failures(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    session_factory = create_session_factory("sqlite+aiosqlite:///:memory:")
    await init_models(session_factory.engine)
    ...
    original_create_research_run = repository.create_research_run

    async def wrapped_create_research_run(input_summary: dict[str, object]):
        run = await original_create_research_run(input_summary)
        created_run_ids.append(run.id)
        return run

    async def fake_run_forecast_workflow(**kwargs: object) -> object:
        raise MarketDataFreshnessError("market data freshness check failed: TradingView 历史日线拉取失败")

    monkeypatch.setattr(repository, "create_research_run", wrapped_create_research_run)
    monkeypatch.setattr("goldfxgraph.research.scheduler.run_forecast_workflow", fake_run_forecast_workflow)
    ...
    assert status.status == "failed"
    assert status.current_stage == "failed"
    assert status.last_error == "market data freshness check failed: TradingView 历史日线拉取失败"
    assert created_run_ids
    run = await repository.get_research_run(created_run_ids[0])
    assert run is not None
    assert run.status == "failed"
    assert run.error_message == "market data freshness check failed: TradingView 历史日线拉取失败"
