...
#### Scenario: Tool 节点执行 TradingView 查价
- **WHEN** workflow 执行 `tool_fetch_current_gold_quote`
- **THEN** 该节点 MUST 通过 TradingView 的 `XAUUSD` 页面获取实时 quote，不得调用 Gold API、旧候选 URL 或任何会回退到旧源的运行时路径

#### Scenario: TradingView quote 不可用
- **WHEN** workflow 执行 `tool_fetch_current_gold_quote` 但 TradingView 页面不可用或解析失败
- **THEN** workflow MUST 返回受控失败或 unavailable 状态，不得用历史 completed daily bar、缓存值或 mock quote 替代 current quote
